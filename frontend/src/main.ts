...
let statusTimer = 0;
let demoMode = location.pathname.replace(/\/$/, '') === '/demo';
...
async function demoApi<T>(path: string, options: RequestInit): Promise<T> {
  ...
  if (Date.parse(demoState.expires_at) <= Date.now()) {
    demoState = null;
    demoWorkspaceId = '';
    sessionStorage.removeItem(DEMO_SESSION_KEY);
    sessionStorage.removeItem(DEMO_STATE_KEY);
    throw new Error('The sample workspace expired after 24 hours. Reset the demo to start again.');
  }
...
function renderLoading(): void {
  ...
  app.innerHTML = shell(`<main id="main" class="loading-view" aria-busy="true"><p class="eyebrow">Opening the dashboard</p><h1>Loading webhook data…</h1><div class="loading-lines" aria-hidden="true"><i></i><i></i><i></i></div></main>`);
  setupTheme();
}

async function startDemo(forceNew = false): Promise<boolean> {
  if (forceNew) {
    demoWorkspaceId = '';
    demoState = null;
    sessionStorage.removeItem(DEMO_SESSION_KEY);
  ...
  if (!demoState) {
    ...
    try {
      ...
      if (!response.ok) throw new Error('The sample workspace could not start.');
      ...
      renderDemoError('The sample workspace could not open. Check your connection, then retry.');
      return false;
    }
  }
  await loadDashboard();
...
async function resetDemo(): Promise<void> {
  ...
  if (await startDemo(true)) showStatus('Sample data reset.');
}
...
function renderDashboard(focusActiveTab = false): void {
  if (!model) return;
  if (demoMode) setPageMeta('Demo — Webhook Quiet Hours', 'Try a temporary webhook failure workspace with realistic sample data.', '/demo');
  else setPageMeta('Dashboard — Webhook Quiet Hours', 'Review webhook fingerprints, aliases, quiet hours, and escalation settings.', '/');
  ...
  app.innerHTML = shell(`<main id="main" class="app-shell"><div class="app-heading"><div><p class="eyebrow">${demoMode ? 'Sample data' : 'Live receiver'}</p><h1>${demoMode ? 'Sample webhook failures' : 'Webhook failures'}</h1></div><div class="connection"><span class="pulse" aria-hidden="true"></span>${demoMode ? 'Temporary workspace' : 'Receiver online'}</div></div><section class="summary-strip" aria-label="Current summary"><article><span>Observed today</span><strong>${model.summary.events_today}</strong></article><article><span>Fingerprints</span><strong>${model.summary.fingerprints}</strong></article><article><span>Repeats compressed</span><strong>${model.summary.compressed}</strong></article><article class="${model.summary.high_unacknowledged ? 'attention' : ''}"><span>High awaiting ack</span><strong>${model.summary.high_unacknowledged}</strong></article></section><div class="workbench"><nav class="tabs" role="tablist" aria-label="Dashboard sections">${tab('observations', 'Observations')}${tab('aliases', `Aliases · ${model.endpoints.length}`)}${tab('settings', 'Quiet rules')}</nav><section id="panel" class="panel" role="tabpanel" aria-labelledby="tab-${activeView}" tabindex="-1"></section></div></main>`);
...
function renderObservations(): void {
  ...
  if (!model.fingerprints.length) {
    panel.innerHTML = `<div class="empty-state"><span class="pressed-leaf" aria-hidden="true">⌁</span><h2>No webhook fingerprints yet</h2><p>Create an alias and send a webhook. Repeated payload shapes will appear here as one fingerprint.</p><button class="primary" id="empty-alias" type="button">Create the first alias</button></div>`;
    document.querySelector('#empty-alias')?.addEventListener('click', () => { activeView = 'aliases'; renderDashboard(); }); return;
  }
  panel.innerHTML = `<div class="panel-heading"><div><h2>Fingerprint ledger</h2><p>${model.summary.pending} failures are waiting for a digest or acknowledgement.</p></div><div class="panel-actions"><button id="export" class="secondary" type="button">Export CSV</button><button id="refresh" class="secondary" type="button">Refresh</button></div></div><ol class="ledger">${model.fingerprints.map((f, index) => fingerprintRow(f, index)).join('')}</ol>`;
...
function openAliasDialog(): void {
  ...
  dialog.querySelector<HTMLFormElement>('form')?.addEventListener('submit', async (event) => {
    event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const data = new FormData(form); const submit = form.querySelector<HTMLButtonElement>('button[type=submit]')!; submit.disabled = true; submit.textContent = 'Creating alias…';
    try {
...
function renderLegal(kind: 'privacy' | 'terms'): void {
  ...
  const body = privacy ? `<p class="effective">Effective 27 August 2026</p><h2>Your instance, your data</h2><p>Webhook Quiet Hours is self-hosted. Your server stores endpoint aliases, event fingerprints, configuration, and retained webhook payloads in its local SQLite database. Payloads, signing secrets, and notification URLs are encrypted at rest with your <code>DATA_ENCRYPTION_KEY</code>.</p><h2>What leaves the instance</h2><p>Configured digests are sent only to the notification URL you provide. License checks send the license token to Sociobot’s billing API. Paid API actions also send that token through your receiver for validation. Sociobot/Dodo acts as merchant of record. This product has no analytics, advertising, cross-tenant telemetry, third-party fonts, or tracking scripts.</p><h2>Deletion and browser storage</h2><p>Retention cleanup deletes individual payloads on your schedule. Deleting an alias removes its observations. The admin token is kept in session storage. A license token and daily verification verdict are kept in local storage and can be removed with your browser controls.</p>` : `<p class="effective">Effective 27 August 2026</p><h2>Use of the software</h2><p>Webhook Quiet Hours is provided under the MIT License. You are responsible for securing the server, setting strong keys, limiting access, and configuring webhook providers correctly. It is an aggregation and alerting aid, not a delivery, retry, or guaranteed incident-response service.</p><h2>One-time purchase</h2><p>The $39 Field Station purchase enables unlimited endpoint aliases and up to 90-day payload retention for this product. Sociobot/Dodo is the merchant of record and handles payment and refunds. A refund revokes the associated license. Core signature verification, high-severity behavior, and export remain available without purchase.</p><h2>Availability and liability</h2><p>The software is provided “as is,” without warranty. You should test notification delivery and maintain independent recovery procedures appropriate to your systems. To the extent permitted by law, the authors are not liable for missed, delayed, or misclassified webhook events.</p>`;
  app.innerHTML = shell(`<main id="main" class="legal"><p class="eyebrow">Legal</p><h1>${privacy ? 'Privacy' : 'Terms of use'}</h1>${body}<p><a class="legal-return" href="/">Return to Webhook Quiet Hours</a></p></main>`); setupTheme();
}

function renderNotFound(): void {
  setPageMeta('Page not found — Webhook Quiet Hours', 'This Webhook Quiet Hours page does not exist.', location.pathname);
  app.innerHTML = shell(`<main id="main" class="not-found"><p class="margin-number">404</p><div><p class="eyebrow">Page not found</p><h1>This page does not exist</h1><p>Check the address, or return to the receiver overview.</p><a class="primary link-button" href="/">Return home</a></div><span class="pressed-leaf" aria-hidden="true">⌁</span></main>`);
  setupTheme();
}
...
async function renderRoute(moveFocus = false): Promise<void> {
  ...
  demoMode = path === '/demo';
  if (path === '/demo') await startDemo();
  else if (path === '/privacy' || path === '/terms') renderLegal(path.slice(1) as 'privacy' | 'terms');
  else if (path === '/') {
    if (token) await loadDashboard(); else renderLanding();
    void processLicense();
  } else renderNotFound();
...
document.addEventListener('click', (event) => {
  ...
  if (url.origin !== location.origin || !['/', '/demo', '/privacy', '/terms'].includes(url.pathname)) return;
  event.preventDefault();
