...
function renderObservations(): void {
  ...
  panel.innerHTML = `<div class="panel-heading"><div><h2>Fingerprint ledger</h2><p>${model.summary.pending} observations are waiting for the next signal.</p></div><div class="panel-actions"><button id="export" class="secondary" type="button">Export CSV</button><button id="refresh" class="secondary" type="button">Refresh</button></div></div><div class="ledger" role="list">${model.fingerprints.map((f, index) => fingerprintRow(f, index)).join('')}</div>`;
